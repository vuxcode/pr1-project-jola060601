...
// Get the correct <img> where the flag is displayed (i.e. the <div> designated with the id="questiontext")
var flagImage = document.querySelector("#stuckwidth img");
// Get the correct <div> where the game modal is displayed
var flagModal = document.querySelector(".game-modal");
// Get the correct <main> where the container box is displayed
var flagContainer = document.querySelector(".containerbox");
...
function getFortyCountries() {
  ...
  while (randomArray.length < 40) {
    ...
    // Check to see if code already exists inside randomArray (use .some() instead of .includes() because the former can check inside an array inside of another array)
    var isDuplicate = randomArray.some(item => item[0] == code);
    // Create an if-statement to check whether isDuplicate is true or false
    if (!isDuplicate) {
      // Save the randomly generated code and country into the empty array
      randomArray.push([code, country]);
    }
...
function clickButtonPoints(clickedButton) {
  ...
  if (clickedButton.innerHTML == currentFlag[1]) {
    correctPoints++;
    ...
    for (var i = 0; i < allButtons.length; i++) {
      // Color the correct button green
      if (allButtons[i].innerHTML == currentFlag[1]) {
...
function cancelGame(clickedButton) {
  ...
  flagModal.querySelector("h2").innerText = `${"Game Over!"}`;
  flagModal.querySelector("button").innerText = `${"Play again"}`
  flagModal.querySelector("p").innerHTML = `${`You cancelled the game before finishing it! You answered ${questionNumber - 1} questions while earning ${correctPoints} points in total.`}`;
}
...
function endGame() {
  ...
  if (correctPoints == 0) {
    ...
    flagModal.querySelector("h2").innerText = `${"Game Over!"}`;
    flagModal.querySelector("button").innerText = `${"Play again"}`
    flagModal.querySelector("p").innerHTML = `${`You answered all ${questionNumber} questions, but you earned ${correctPoints} points in total.`}`;
  }
  else if (correctPoints == maxPoints) {
    // Change the text displayed on the game modal
    flagModal.querySelector("h2").innerText = `${"Congratulations!"}`;
    flagModal.querySelector("button").innerText = `${"Play again"}`
    flagModal.querySelector("p").innerHTML = `${`You answered all ${questionNumber} questions, and you earned all ${correctPoints} points in total.`}`;
  }
  else {
    // Change the text displayed on the game modal
    flagModal.querySelector("h2").innerText = `${"Finish!"}`;
    flagModal.querySelector("button").innerText = `${"Play again"}`
    flagModal.querySelector("p").innerHTML = `${`You answered all ${questionNumber} questions, and you earned ${correctPoints} points in total.`}`;
  }
}
...
function startGame() {
  ...
  // Add country names to the buttons
  ...
  flagImage.src = `flags/${currentFlag[0]}.svg`;
